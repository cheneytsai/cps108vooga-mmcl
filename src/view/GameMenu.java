        ...
        MouseAdapter myMouseAdapter = new MouseAdapter()
        {
            public void mouseClicked(MouseEvent e)
            {
                        ...
                        && e.getX() < 375 + 12 * AVG_PIXELS_PER_LETTER)
                {
                    ...
                    {
                        File folder = new File("src/" + myGameName
                                + "/savedGames");
                        ...
                        String fileName = (String) JOptionPane.showInputDialog(
                                null, "Pick a game to play:", "New Game Mode",
                                JOptionPane.PLAIN_MESSAGE, null, fileNames,
                                null);
                        if (fileName == null)
                        {
                            JOptionPane.showMessageDialog(null,
                                    "You have not chosen a game to play!",
                                    "Error", 0);
                        } else if (fileName.equals("New Game"))
                        {
                            Reflection.createInstance(myGameName.toLowerCase()
                                    + "." + myGameName + "Model", myGameName,
                                    "", DEFAULT_START_LEVEL, myGameName
                                            .toLowerCase()
                                            + "." + myGameName + "LevelViewer",
                                    myCanvas);
                        } else
                        {
                            ...
                            {
                                // For PCs
                                Reflection.createInstance(myGameName
                                        .toLowerCase()
                                        + "." + myGameName + "Model",
                                        myGameName, fileName.replace(
                                                "src\\" + myGameName
                                                        + "\\savedGames\\", "")
                                                .replace(".txt", ""),
                                        DEFAULT_START_LEVEL, myGameName
                    ...
                    } else if (e.getY() > 420 && e.getY() < 460)
                    {
                        ...
                        try
                        {
                            ...
                            Reflection.createInstance(myGameName.toLowerCase()
                                    + "." + myGameName + "Model", myGameName,
                                    "", showLevel, "view.EditorCanvas",
                                    myCanvas);
                        } catch (NullPointerException n)
                        {
                            JOptionPane.showMessageDialog(null,
                                    "You have not chosen a game to edit!",
                                    "Error", 0);
                        }
