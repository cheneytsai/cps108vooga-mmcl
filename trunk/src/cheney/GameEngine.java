...
public class GameEngine {
    ...
    public void addActor(String[] actorData) {
        try
        {
        Constructor<?> con = Class.forName(actorData[0]).getConstructor(String[].class, GameEngine.class);
        Object[] params = {actorData, this};
    ...
    public void addInteraction(List<String[]> conditionsData, List<String[]> actionsData){
        ...
        try
        {
            for (String[] conditionData : conditionsData)
            {
                ...
                Constructor<?> con = Class.forName(conditionData[0]).getConstructor(String[].class, GameEngine.class);
                Object[] params = {conditionData, this};
                conditions.add((Condition) con.newInstance(params));
            }   

            for (String[] actionData : actionsData)
            {
                Constructor<?> con = Class.forName(actionData[0]).getConstructor(String[].class, GameEngine.class);
                Object[] params = {actionData, this};
